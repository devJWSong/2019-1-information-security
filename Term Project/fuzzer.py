...
afl_path = '/mnt/c/Users/enflw/Project/afl-2.52b'
...
interest_list = [0, 0xffffffff, 0x7fffffff] # interesting values example
...
def mutate(orig_file):
    ...
    for j in range(mutation_count):
        ...
        elif strategy == 1:                                 # random increment
            inc_size = random.randrange(1, 256)
            mutate_data = mutate_data[:offset_to_mutate] + chr((ord(mutate_data[offset_to_mutate]) + inc_size) % 256) + mutate_data[offset_to_mutate + 1:]

        elif strategy == 2:                                 # random xor with interesting value
            for i in range(0, in_size):
                interesting_index = random.randrange(0, 3)
                mutate_data = mutate_data[:i] + chr((ord(mutate_data[i]) ^ interest_list[interesting_index]) % 256) + mutate_data[i + 1:]

        elif strategy == 3:                                 # byte flipping
            b = random.randrange(1, 256)
            char = mutate_data[offset_to_mutate]
            mutate_data = mutate_data[:offset_to_mutate] + chr((ord(char) ^ b)) + mutate_data[offset_to_mutate + 1:]

        elif strategy == 4:                                 # bit flipping
            char = mutate_data[offset_to_mutate]
            bit_index = random.randrange(0, 7)
            bit = 1
            bit = bit << bit_index

            mutate_data = mutate_data[:offset_to_mutate] + chr((ord(char) ^ bit)) + mutate_data[offset_to_mutate + 1:]

        elif strategy == 5:                                 # block insertion
            temp = ""

            for i in range(rand_size):
                temp += chr(random.randrange(0, 16))

            mutate_data = mutate_data[:offset_to_mutate] + temp + mutate_data[offset_to_mutate+1:]
